...
class Cursor {
    ...
    cardStackSelectNextInDirection(layout, directionSpecified) {
        ...
        var displacementToStackOther = new Coords(0, 0);
        var directionToStackOther = new Coords(0, 0);
        ...
        for (var i = 0; i < stacks.length; i++) {
            ...
            if (stackOther != stackCurrent) {
                displacementToStackOther.overwriteWith(stackOther.pos).subtract(stackCurrent.pos);
                var distanceToStackOther = displacementToStackOther.magnitude();
                directionToStackOther.overwriteWith(displacementToStackOther).divideScalar(distanceToStackOther);
                var dotProduct = directionToStackOther.dotProduct(directionSpecified);
                if (dotProduct > 0) {
        ...
        if (cardStackSelected != null) {
            if (this.cardStackBeingMoved == null) {
                if (cardStackSelected.cards.length == 0) {
                    ...
                    this.cardStackBeingMoved = cardStackSelected.drawCardsAsCardStack(this.numberOfCardsSelected);
                    this.numberOfCardsSelected = 0;
                }
            }
            else {
    ...
    dropCardStackBeingMovedOntoOther(cardStackSelected) {
        ...
        this.cardStackBeingMoved = null;
        this.numberOfCardsSelected = 0;
        if (this.cardStackBeingMovedFrom.defn().showTopCardAfterMove) {
    ...
    cardStackSelectionCountAdd(offset) {
        if (this.cardStackIndexSelected != null
            ...
            if (numberOfCardsSelectedNext >= 0
