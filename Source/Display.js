...
class Display {
    ...
    drawCardAtPos(card, pos) {
        ...
        this.graphics.fillRect(pos.x, pos.y, cardSizeInPixels.x, cardSizeInPixels.y);
        this.graphics.strokeStyle = "LightGray";
        this.graphics.strokeRect(pos.x, pos.y, cardSizeInPixels.x, cardSizeInPixels.y);
        if (isFaceUp == true) {
    ...
    drawCardStack(cardStack) {
        ...
        this.graphics.strokeStyle = "LightGray";
        this.graphics.strokeRect(drawPos.x, drawPos.y, cardSizeInPixels.x, cardSizeInPixels.y);
        this.graphics.beginPath();
        this.graphics.moveTo(drawPos.x, drawPos.y);
        this.graphics.lineTo(drawPos.x + cardSizeInPixels.x, drawPos.y + cardSizeInPixels.y);
    ...
    drawCursor(cursor) {
        if (cursor.cardStackSelected() != null) {
            if (cursor.cardStackBeingMoved == null) {
                ...
                var cardStackToHighlight = cursor.cardStackSelected().topCardsAsCardStack(numberOfCardsSelected);
    ...
    drawHighlightForCardStack(cardStackToHighlight) {
        ...
        if (numberOfCardsToHighlight == 0) {
            ...
            this.graphics.fillText(numberOfCardsToHighlight, highlightPos.x, highlightPos.y + highlightSize.y);
        }
        this.graphics.strokeRect(highlightPos.x, highlightPos.y, highlightSize.x, highlightSize.y);
    }
    drawCursor_2(cursor) {
        ...
        cardStackBeingMoved.pos = cardStackSelectedPos.clone().add(cardStackSelectedSpacing.multiplyScalar(cardStackSelected.cards.length)).add(offset);
