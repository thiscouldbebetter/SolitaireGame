    ...
    static solitaire() {
        ...
        var cardStackDefns = [
            ...
            (cursor, cardStackAccepting) => {
                ...
                var rankValueOfCardToAccept = Rank.byCode(rankCodeOfCardToAccept).value;
                if (cardStackToAccept.cards.length > 1) {
                    ...
                    var rankValueOfCardAccepting = Rank.byCode(rankCodeOfCardAccepting).value;
                    var rankOfCardAcceptingMinusAccepted = rankValueOfCardAccepting
                        - rankValueOfCardToAccept;
                    var isRankOfCardAcceptingOneLessThanAccepted = (rankOfCardAcceptingMinusAccepted == -1);
                    canDrop =
                        (doSuitsMatch
                            && isRankOfCardAcceptingOneLessThanAccepted);
                }
                if (canDrop == true) {
                    cursor.dropCardStackBeingMovedOntoOther(cardStackAccepting);
            ...
            new CardStackDefn("Stock", 1, // cardsSelectableMax
            ...
            new CardStackDefn("Tableau", null, // cardsSelectableMax
            false, // areFaceDownCardsSelectable
            true, // showTopCardAfterMove
            // spacingBetweenCards
            ...
            function (cursor, cardStackAccepting) {
                ...
                if (cardStackAccepting.cards.length == 0) {
                    ...
                    var rankOfCardAcceptingMinusAccepted = rankValueOfCardAccepting
                        - rankValueOfCardToAccept;
                    var isRankOfCardAcceptingOneGreaterThanAccepted = (rankOfCardAcceptingMinusAccepted == 1);
                    canDrop =
                        (areColorsDifferent
            ...
            new CardStackDefn("Waste", 1, // cardsSelectableMax
            false, // areFaceDownCardsSelectable
            true, // showTopCardAfterMove
            // spacingBetweenCards
            new Coords(0, -.4), 
        ...
        var cardStackDefnsByName = new Map(cardStackDefns.map(x => [x.name, x]));
        var actions = [
            new Action("TakeOrDrop", "Enter", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectedTakeOrDrop();
            }),
            new Action("Cancel", "Escape", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectedDeselect();
            }),
            new Action("CountIncrement", "ArrowUp", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectionCountAdd(1);
            }),
            new Action("CountDecrement", "ArrowDown", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectionCountAdd(-1);
            }),
            new Action("MoveLeft", "a", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectNextInDirection(session.layout, new Coords(-1, 0));
            }),
            new Action("MoveRight", "d", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectNextInDirection(session.layout, new Coords(1, 0));
            }),
            new Action("MoveDown", "s", // keyCode,
            // performForSession
            (session) => {
                session.cursor.cardStackSelectNextInDirection(session.layout, new Coords(0, 1));
            }),
            new Action("MoveUp", "w", // keyCode
            // performForSession
            (session) => {
                session.cursor.cardStackSelectNextInDirection(session.layout, new Coords(0, -1));
            }),
        ...
        var layoutBuild = () => {
            var deck = CardStack.fromCardDefns(null, // name
            null, // defnName
            null, // pos
            cardDefnSetStandard.cardDefns).shuffle();
            var cardStacks = [];
            var numberOfTableaus = 7;
            var tableauPos = new Coords(cardSizeInPixels.x, cardSizeInPixels.y * 2);
            var spacingBetweenTableaus = new Coords(cardSizeInPixels.x * 1.5, 0);
            for (var t = 0; t < numberOfTableaus; t++) {
                var cardsInTableau = numberOfTableaus - t;
                var cardStackTableau = new CardStack("Tableau" + t, cardStackDefnsByName.get("Tableau").name, tableauPos.clone(), deck.drawCardsFaceDown(cardsInTableau));
            ...
            var foundationPos = new Coords(cardSizeInPixels.x, cardSizeInPixels.y / 2);
            var spacingBetweenFoundations = new Coords(cardSizeInPixels.x * 1.5, 0);
            for (var s = 0; s < numberOfFoundations; s++) {
                var cardStackFoundation = new CardStack("Foundation" + s, cardStackDefnsByName.get("Foundation").name, foundationPos.clone(), [] // cards
                );
            ...
            var cardStackStock = new CardStack("Stock", cardStackDefnsByName.get("Stock").name, stockPos, deck.cards);
            cardStacks.push(cardStackStock);
            var wastePos = stockPos.clone().add(spacingBetweenFoundations);
            var cardStackWaste = new CardStack("Waste", cardStackDefnsByName.get("Waste").name, wastePos, [] // cards
            );
            cardStacks.push(cardStackWaste);
            var layout = new Layout(cardStacks);
            return layout;
        };
        var returnValue = new GameDefn("Solitaire", cardDefnSetStandard, cardStackDefns, actions, layoutBuild);
        return returnValue;
    }
}
